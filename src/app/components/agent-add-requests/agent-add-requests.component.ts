...
@Component({
  selector: 'app-agent-add-requests',
  templateUrl: './agent-add-requests.component.html',
  styleUrls: ['./agent-add-requests.component.css']
})
export class AgentAddRequestsComponent implements OnInit {
  ...
  requests:any[] = [];
  displayedColumns: string[] = ['vid','companyName','aid','firstName','type','date','accept','reject'];
  pageNo:number = 0;
  pageSize:number = 10;
  config:any = {};
  constructor(
  ...
  getRequests(){
    ...
    this.agentService.fetchAllRequests("addition",this.pageNo,this.pageSize).subscribe(res=>{
